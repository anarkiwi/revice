...
# 01-monitor-lexer-parser.patch edits mon_lex.l / mon_parse.y but (per its commit
# message) intentionally does NOT carry the generated mon_lex.c / mon_parse.{c,h}
# — autogen + make regenerate them. A VICE tree, however, ships those generated
# files committed and *stale* (they predate the new keymatrix/screenscrape
# tokens). On a fresh checkout the generated files and the .l/.y sources get
# near-equal mtimes, so make non-deterministically decides the stale generated
# files are up to date and skips regeneration -> the build fails with
# "'CMD_KEYMATRIX' undeclared" (or silently drops the new commands). Delete the
# generated files here so the subsequent build always regenerates them from the
# patched sources. BUILT_SOURCES + the `mon_parse.h: mon_parse.c` rule keep the
# regeneration race-free even under `make -j`.
echo "Forcing regeneration of monitor lexer/parser (removing stale generated files)..."
for g in mon_lex.c mon_parse.c mon_parse.h; do
    f="$VICE_ROOT/src/monitor/$g"
    if [ -f "$f" ]; then
        rm -f "$f"
        echo "  removed: src/monitor/$g (will be regenerated by make)"
    fi
done
